...
async def query(request: QueryRequest) ->QueryResponse:
    ...
    try:
        ...
        ## Determine which request to call based on request
        if request.enable_evaluation:
            ## evaluation requires sources, so we always include then
            result = await rag_chain.aquery_with_evaluation(
                question=request.question,
                included_sources=request.include_sources
            )
